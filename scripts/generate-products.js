...
// ── 48 unique verified-working Unsplash images (one per product) ──
const UNIQUE_IMAGES = [
...
function getUniqueImage() {
  const id = UNIQUE_IMAGES[imageIndex % UNIQUE_IMAGES.length];
  imageIndex++;
...
for (const [catName, catData] of Object.entries(CATEGORIES)) {
  ...
  for (const type of catData.types) {
    if (typeCount >= 6) break; // max 6 types per category = 48 total
    typeCount++;
    ...
    // No extra variants — one product per type, each with unique image
  }
}
