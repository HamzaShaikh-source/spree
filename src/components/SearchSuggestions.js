...
export default function SearchSuggestions({ query, onSearch, onSubmit }) {
  ...
  if (!show && query.length === 0 && searchHistory.length === 0) return null;
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
        ...
        {suggestions.length > 0 && (
          <div>
            ...
            {suggestions.map(p => (
              <button key={p.id} onClick={() => handleSelect(p.name)}
        ...
        {query.length === 0 && searchHistory.length > 0 && (
          <div className="p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={clearHistory} className="text-[10px] text-red-400 hover:text-red-500">Clear</button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {searchHistory.map((term, i) => (
                <button key={i} onClick={() => handleHistoryClick(term)}
